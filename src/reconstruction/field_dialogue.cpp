...
[[noreturn]] void music_call(std::uint32_t address) {
    std::string id = "symbol:music-call-";
    for (auto shift = 28; shift >= 0; shift -= 4)
        id += "0123456789abcdef"[(address >> static_cast<unsigned>(shift)) & 15U];
    throw MissingDependency({"music_call", address, {}, {}}, id, false,
                            "This music call is not connected to Program");
}

// The calls field 80085560 makes, on Program state; the others stop.
class ProgramMusicCalls final : public field::UnrecoveredMusicCalls {
  public:
    explicit ProgramMusicCalls(Program &owner) : program(owner) {}
    // Resident 8002a260: allocate count * 808 + 24 bytes, store the count,
    // then select (80028a94) and reset (80028aac) the ring.
    field::MusicResource allocate_stream_buffer(std::uint32_t blocks, std::uint32_t mode) override {
        auto &resident = program.resident;
        if (s32(blocks) < 1)
            return 0;
        auto block =
            resident::heap_allocate(resident.heap, blocks * 0x808U + 0x24U, mode, 0x8002a284);
        if (!block)
            return 0;
        auto &ring = resident.disc_read.ring;
        if (!ring.bytes.empty())
        ...
        ring = {block->address, {block->bytes.begin(), block->bytes.begin() + header}};
        resident.music_blocks.push_back({block->address + static_cast<std::uint32_t>(header),
        ...
        static_cast<void>(field::reset_disc_stream_ring(resident.disc_stream, ring.bytes));
        return block->address;
    }
    // Resident 800295d8; 80085560 ignores the result.
    void read_file(std::uint32_t file, field::MusicResource destination, std::uint32_t offset,
                   std::uint32_t mode) override {
        static_cast<void>(program.read_file(s32(file), destination, offset, mode));
    }
    field::MusicResource next_stream_chunk() override { music_call(0x80028b14); }
    void release_stream_chunk(field::MusicResource) override { music_call(0x8002945c); }
    field::MusicResource start_wave_transfer(field::MusicResource, std::uint32_t,
                                             std::uint32_t) override {
        music_call(0x800380d0);
    }
    void continue_wave_transfer(field::MusicResource, std::uint32_t) override {
        music_call(0x8003827c);
    }
    void wait_audio_service(std::uint32_t) override { music_call(0x8003bdfc); }
    void release_buffer(field::MusicResource) override { music_call(0x800320e8); }
    void select_directory(std::uint32_t, std::uint32_t) override { music_call(0x80028470); }
...
void Program::load_music(std::uint32_t id) {
    ...
    if (bank != 0xff && bank != music.loaded_wave_bank) {
        ProgramMusicCalls calls(*this);
        // 80085560 with the chunk callback 800859dc.
        field::start_music_stream(
            music.stream, resident.battle_request, bank * 2U + 0x13U, 1, 0x800859dc,
            [](field::MusicResource) {
                throw MissingDependency({"music_chunk", 0x800859dc, {}, {}},
                                        "symbol:field-music-chunk-800859dc", true,
                                        "The wave chunk callback is not connected to Program");
            },
            calls);
