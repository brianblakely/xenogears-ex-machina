...
constexpr std::uint32_t key_off = 0x80059550;
...
class Tick {
  ...
  private:
    ...
    void keys() {
        ...
        const auto off = m.u32(key_off) | m.d.voice_changes;
        if (off != 0) {
    ...
    // 8003eea0: the SPU pitch of a key in 1/256 semitones.
    std::uint32_t pitch(std::uint32_t key) {
        const auto octave = m.u8(0x80050b78 + ((key & 0x7fffU) >> 8U));
        const auto index = ((key & 0xffU) + ((octave & 0xfU) << 8U)) << 1U;
        auto value = u(s16(m.u16(0x80050bf0 + index)));
        const auto shift = 6 - s32(octave >> 4U);
        value = shift < 0 ? value << u(-shift) : sra(value, u(shift));
        return u(s16(value));
    }

    // 8003ebf0: compute each changed voice's volume pair and pitch, then key
    // it on or off.
    void mix(std::uint32_t seq, std::uint32_t voices, std::uint32_t count) {
        if ((m.u16(seq + 0x10) & 0x20U) != 0)
            return;
        for (auto voice = voices; count != 0; --count, voice += voice_stride) {
            if (m.u16(voice) == 0)
                continue;
            const auto changes = m.u16(voice + 2);
            const auto owner = voice + 0x30;
            if ((changes & 0x100U) != 0) {
                const auto base = u(s16(m.u16(voice + 0x7a)));
                auto level = base - sra(mul(base, u(s16(m.u16(voice + 0xd2)))), 15);
                if (s32(level) > 0x7fff)
                    level = 0x7fff;
                if (s32(level) < 0)
                    level = 0;
                level = sra(mul(u(s16(m.u16(voice + 0x76))), level), 15);
                level = sra(mul(u(s16(m.u16(seq + 0x72))), level), 16);
                auto pan = u(s16(m.u16(voice + 0x74))) + u(s16(m.u16(voice + 0xd4))) +
                           u(s16(m.u16(seq + 0x8a)));
                if (s32(pan) > 0x7f00)
                    pan = 0x7f00;
                if (s32(pan) < 0)
                    pan = 0;
                ...
                if ((m.d.flags & 0x100U) != 0) {
                    const auto near = [](std::uint32_t x) { return sra((x * 45U) << 9U, 14); };
                    const auto far = [](std::uint32_t x) {
                        return 0x7f00U - sra((x * 37U) << 8U, 14);
                    };
                    if (s32(pan) < 0x4000) {
                        right = near(pan);
                        left = far(pan);
                    } else {
                        const auto mirrored = 0x8000U - pan;
                        left = near(mirrored);
                        right = far(mirrored);
                    }
                    left = sra(mul(left, level), 15);
                    right = sra(mul(right, level), 15);
                } else {
                    right = sra((level * 45U) << 9U, 15);
                    left = right;
                }
                m.w16(voice + 0x38, left);
                m.w16(voice + 0x3a, right);
                m.w16(voice + 0x36, m.u16(voice + 0x36) | 1U);
            }
            if ((changes & 0x200U) != 0) {
                const auto key = u(s16(u(s16(m.u16(voice + 0x6a))) + u(s16(m.u16(voice + 0xd0))) +
                                       u(s16(m.u16(seq + 0x7e)))));
                m.w16(voice + 0x44, pitch(key) & 0x3fffU);
                m.w16(voice + 0x36, m.u16(voice + 0x36) | 4U);
            }
            if ((changes & 1U) != 0 && (m.u16(voice) & 0x20U) == 0)
                key_on(owner, m.u8(voice + 0x27));
            if ((changes & 2U) != 0)
                key_release(owner, m.u8(voice + 0x27));
            m.w16(voice + 2, 0);
        }
    }

    // 8003ef04: claim hardware voice `channel` for `owner` (unless a holder
    // has a higher priority) and key it on.
    void key_on(std::uint32_t owner, std::uint32_t channel) {
        if (channel >= 24)
            return;
        const auto holder = m.d.voice_owners[channel];
        if (holder != owner) {
            if (holder != 0 && s16(m.u16(owner + 4)) < s16(m.u16(holder + 4)))
                return;
            m.w16(owner + 6, 0xffff);
            m.w16(owner, channel);
            m.d.voice_owners[channel] = owner;
            m.d.voice_changes |= 1U << channel;
        }
        m.d.voice_holds |= 1U << channel;
    }
    // 8003efa0: key off `channel` if `owner` holds it.
    void key_release(std::uint32_t owner, std::uint32_t channel) {
        if (channel < 24 && m.d.voice_owners[channel] == owner)
            m.w32(key_off, m.u32(key_off) | (1U << channel));
    }
};
} // namespace
