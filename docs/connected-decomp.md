...
behavior. A new `ConnectedField` owns its event package, actors, descriptor flags,
variable bank, control/pass state and explicitly supplied battle/media boundary
values. Borrowed views exist only during a call, so moving a prepared state cannot
leave pointers into its former owner.
