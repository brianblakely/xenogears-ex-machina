...
void opening() {
    ...
    resident.sound.held[0x800694f8] = {0}; // The pool byte 800694f8 is clear.
    ...
    for (std::uint32_t i = 0; i < 16; ++i) {
        ...
        gpu.vram_reads.emplace_back(0x18c, static_cast<std::uint8_t>(i));
    }
...
int main() {
    try {
        ...
        stage_setup();
        opening();
        std::cout << "Battle actions: twelve source-boundary groups passed\n";
