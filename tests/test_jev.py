...
from tools.analysis import jev
...
def inventory_fixture():
    def table(namespace, opcodes):
        ...
        return {
            ...
            "rows": [{"opcode": f"0x{opcode:02x}", "handler": "0x80010000",
                      "analysis_status": "unresolved", "native_status": "unimplemented"}
    ...
    return {
        "schema_version": 1, "coverage_complete": False, "scope": "Synthetic only",
        "reconstruction_status_rule": "No promotion", "unknown_instruction_policy": "Fail",
        "unresolved_symbols": [], "unknown_formats": [], "unverified_behavior": [],
...
def response(score=0.8):
    return {"model": jev.MODEL, "answers": {"relevance": {"type": "noul", "noul": score}},
...
class JevTests(unittest.TestCase):
    def setUp(self):
        ...
        self.report = {"status": "dependency_needs_recovery",
                       ...
                       "source": self.source, "partial_state": "NEVER_UPLOAD_THIS",
                       "checkpoints": ["EXPECTED_BYTES"], "case": "PRIVATE_CASE"}
    ...
    def rank(self, **kwargs):
        return jev.rank(self.root, self.blocker, "Review task list ownership", self.rows,
                        online=kwargs.pop("online", True),
                        allow_private=kwargs.pop("allow_private", False), **kwargs)
    ...
    def test_conflicting_code_identity_excludes_finding_before_ranking(self):
        self.write("analysis/findings/conflict.json", {
    ...
    def test_stale_targets_and_corrupted_inventory_fail(self):
        with self.assertRaisesRegex(ValueError, "Stale"):
            jev.blocker_from_report({**self.report, "recovery": {"table_value": "0x80020000"}},
                                    self.inventory)
    ...
    def test_namespaces_and_unlisted_dependencies_remain_distinct(self):
        primary = jev.blocker_from_report({**self.report, "dependency": "instruction:primary:0x96"},
                                         self.inventory)
        self.assertNotIn("next_experiment", primary["inventory"])
        unlisted = jev.blocker_from_report({**self.report, "dependency": "symbol:unknown"},
                                          self.inventory)
    ...
    def test_allowlist_and_line_exact_excerpts(self):
        ...
        self.assertEqual(row["excerpt"], "".join(data.decode().splitlines(keepends=True)[
    ...
    def manifest(self, source=None, digest=None):
        ...
        self.write(".local/manifest.json", {"artifacts": [{"path": ".local/export.c",
                   "source": source or self.source, "sha256": digest or jev.sha(data)}]})
    ...
    def test_private_excerpt_is_labeled_and_never_silently_uploaded(self):
        ...
        with patch.object(jev, "post") as post:
    ...
    def test_offline_and_missing_key_make_no_requests(self):
        with patch.dict(os.environ, {}, clear=True), patch.object(jev, "post") as post:
            self.assertEqual(self.rank()["method"], "local_fallback")
            self.assertEqual(self.rank(online=False)["method"], "local")
            post.assert_not_called()

    def test_valid_online_ranking_and_cache_replay_without_key(self):
        def service(payload, key):
        ...
        with (
            ...
            patch.object(jev, "post", side_effect=service) as post,
        ):
            result = self.rank()
    ...
    def test_changed_question_excerpt_or_model_cannot_reuse_cache(self):
        ...
        with (
            ...
            patch.object(jev, "post", side_effect=lambda *a: response()) as post,
        ):
            self.rank()
            self.rows[0]["excerpt"] += " changed"
            self.rank()
            jev.rank(
                self.root, self.blocker, "different question", self.rows,
                online=True, allow_private=False,
            )
    ...
    def test_corrupted_cache_is_ignored_not_trusted(self):
        ...
        with (
            patch.dict(os.environ, {"TYPESAFE_API_KEY": "test"}),
            patch.object(jev, "post", return_value=response()) as post,
        ):
            self.rank()
    ...
    def test_partial_failure_preserves_whole_local_order_and_redacts_error(self):
        ...
        with (
            patch.dict(os.environ, {"TYPESAFE_API_KEY": "test-secret"}),
            patch.object(jev, "post", side_effect=service),
        ):
            result = self.rank()
    ...
    def test_proposals_are_bounded_bound_to_source_and_preserve_ids(self):
        doc = {"source": self.source, "question": "Which experiment distinguishes A and B?",
               "candidates": [{"id": "observe-init", "description": "Observe list initialization"},
        ...
        with patch.object(jev, "post") as post:
            result = jev.rank(
                self.root, self.blocker, question, rows,
                online=False, allow_private=False, choices=True,
            )
            post.assert_not_called()
    ...
    def test_http_wire_shape_timeout_and_redirect_rejection(self):
        ...
        with patch.object(jev.urllib.request, "build_opener", return_value=Opener()):
            self.assertEqual(jev.post({"test": 1}, "test"), response())
        self.assertIsNone(
            jev.NoRedirect().redirect_request(None, None, 302, None, {}, "https://other.invalid")
        )
    ...
    def test_http_overload_retries_once_and_respects_long_retry_after(self):
        for delay, attempts in (("0", 2), ("99", 1), ("unrecognized", 1)):
            ...
            opener = Opener()
            with (
                patch.object(jev.urllib.request, "build_opener", return_value=opener),
