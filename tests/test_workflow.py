...
class EvidenceWorkflowTests(unittest.TestCase):
    def test_lettered_phase_and_duplicate_contract_tables_remain_distinct(self) -> None:
        plan = (
            ...
            "## Phase 2A — Authoring\n- [x] Authored task.\n"
            "**Exit criterion:** Complete bridge ready.\n"
        ...
        self.assertEqual(
            source["exits"], {"P02": "Runtime ready.", "P02A": "Complete bridge ready."}
        )
        self.assertNotIn("early_exits", source)
        self.assertEqual(
            source["checked_tasks"], {"P02-T01": True, "P02A-T01": True, "P03-T01": False}
        )
        rows = [
            {"source_id": "P02-T01", "status": "passed"},
            {"source_id": "P02A-T01", "status": "defined"},
        ]
    ...
    def test_authoring_bridge_has_an_ordinary_complete_phase(self) -> None:
        matrix = build_matrix()
        authoring = [row for row in matrix["requirements"] if row["phase"] == "2A"]
        self.assertEqual(len({row["source_id"] for row in authoring}), 36)
        self.assertTrue(
            all(row["status"] == "defined" and not row["evidence"] for row in authoring)
        )
        self.assertEqual(target_scope("P02A-T30"), ["arch-vulkan", "arch-headless"])
        self.assertIn("windows-headless", target_scope("P02A-T07"))
        self.assertIn("macos-metal", target_scope("P13-T19"))
        gate = next(g for g in matrix["crosscutting"]["phase_exits"] if g["phase"] == "2A")
        self.assertTrue(gate["requires_all_phase_facets"])
        self.assertEqual(gate["requires_previous_phase"], "P02")
        self.assertEqual(set(gate["tasks"]), {row["source_id"] for row in authoring})
    ...
    def test_authoring_source_tables_prose_and_exit_cannot_drift(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "foundational authoring requirement"):
            validate_traceability(matrix)
        matrix = build_matrix()
        next(g for g in matrix["crosscutting"]["phase_exits"] if g["phase"] == "2A")["source"] = (
            "lost"
        )
        with self.assertRaisesRegex(ValueError, "phase exit criterion"):
            validate_traceability(matrix)

    def test_early_exit_exceptions_are_rejected(self) -> None:
        with self.assertRaisesRegex(ValueError, "Early"):
            plan_sources(
                "## Phase 2A — Bridge\n**Early exit criterion, required by Phase 3:** Partial.\n"
            )
        matrix = build_matrix()
        matrix["crosscutting"]["early_exits"] = []
        with self.assertRaisesRegex(ValueError, "Early-exit exceptions"):
            validate_traceability(matrix)
        matrix = build_matrix()
        gate = next(g for g in matrix["crosscutting"]["phase_exits"] if g["phase"] == "2A")
        gate["requires_all_phase_facets"] = False
        with self.assertRaisesRegex(ValueError, "must require all its facets"):
            validate_traceability(matrix)

    def test_phase3_cannot_pass_without_completed_authoring_phase(self) -> None:
        matrix = build_matrix()
        ...
        for row in matrix["requirements"]:
            if row["phase"] == 3:
                row["status"] = "passed"
                matrix["source_snapshot"]["checked_tasks"][row["source_id"]] = True
        with self.assertRaisesRegex(ValueError, "cannot precede completion of its prerequisite"):
            validate_traceability(matrix)
        phase3["requires_previous_phase"] = "P02"
        with self.assertRaisesRegex(ValueError, "immediately preceding completed phase"):
            validate_traceability(matrix)

    def test_release_and_phase_gate_cannot_omit_required_scope(self) -> None:
        matrix = build_matrix()
        next(g for g in matrix["crosscutting"]["phase_exits"] if g["phase"] == "2A")["tasks"].pop()
        with self.assertRaisesRegex(ValueError, "Phase gate omits checklist tasks"):
            validate_traceability(matrix)
        matrix = build_matrix()
        preview = next(
            gate
        ...
        preview["required_phases"].remove("2A")
        with self.assertRaisesRegex(ValueError, "full sequential prefix"):
            validate_traceability(matrix)
        matrix = build_matrix()
    ...
    def test_checked_work_requires_all_previous_phase_checkboxes(self) -> None:
        for earlier in ("- [ ] Earlier work.\n", ""):
            plan = "## Phase 2 — Earlier\n" + earlier + "## Phase 2A — Later\n- [x] Later work.\n"
            rows = [{"source_id": "P02A-T01", "status": "passed"}]
            with self.subTest(earlier=earlier), self.assertRaisesRegex(ValueError, "earlier phase"):
                validate_plan_completion(plan, rows)

    def test_phase_pass_and_complete_checklist_are_equivalent(self) -> None:
        for declared_pass in (True, False):
            matrix = build_matrix()
            gate = next(g for g in matrix["crosscutting"]["phase_exits"] if g["phase"] == 2)
            for row in matrix["requirements"]:
                if row["phase"] == 2:
                    row["status"] = "passed"
                    matrix["source_snapshot"]["checked_tasks"][row["source_id"]] = not declared_pass
            if declared_pass:
                gate.update(status="passed", evidence=["synthetic-gate-test-only"])
            with (
                self.subTest(declared_pass=declared_pass),
                self.assertRaisesRegex(ValueError, "must equal every checkbox"),
            ):
                validate_traceability(matrix)

    def test_integrated_acceptance_is_last_check(self) -> None:
        matrix = build_matrix()
        gate = next(g for g in matrix["crosscutting"]["phase_exits"] if g["phase"] == 2)
        matrix["source_snapshot"]["checked_tasks"][gate["completion_task"]] = True
        with self.assertRaisesRegex(ValueError, "Integrated acceptance cannot precede"):
            validate_traceability(matrix)

    def test_later_phase_cannot_record_executed_work_before_prerequisite(self) -> None:
        for status in ("passed", "failed"):
            matrix = build_matrix()
            next(r for r in matrix["requirements"] if r["phase"] == 3)["status"] = status
            with (
                self.subTest(status=status),
                self.assertRaisesRegex(ValueError, "cannot precede completion"),
            ):
                validate_traceability(matrix)

    def test_current_phase_can_record_work_without_claiming_completion(self) -> None:
        matrix = build_matrix()
        next(r for r in matrix["requirements"] if r["phase"] == 1)["status"] = "failed"
        validate_traceability(matrix)
