...
# StoreImage read-backs without a service hook or a recorded VRAM read-back:
# the hook after the transfer completes, then per read-back in call order the
# word naming the block, the destination's offset in it and the byte count.
# The battle's 80077990 reads four palette rows (198 x 1) four times each
# into the graphics block (800c3ea4) before op-camera.
VRAM_READS = {
    "op-camera": [
