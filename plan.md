...
Reusable C++ source now connects packed field/event/collision parsing, actor defaults and initial floor queries, the event core, control requests, planar/jump stages, sprite construction/animation/checkpoints and the supported factory subset, battle requests and continuation, snapshot data restore, music caller/wave policy and the retail CD ring. Scheduler/control/motion share the original pass state, and supported ordinary sprite commands invoke the recovered motion arithmetic. Call/return handlers have additional source/synthetic coverage only. The original encounter/victory/return/menu input route and checkpoints are fixed in the slice manifest, with required extensions and alternatives still unresolved. Required NPC factory commands/tasks, cleanup, full motion/collision/position, heap/CD producers/audio internals and the remaining domain requirements stay open. No broad Phase 1 todo or complete proof domain is closed by this source milestone.
