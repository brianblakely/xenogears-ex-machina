...
Reusable C++ source now connects packed field/event/collision parsing, actor defaults and initial floor queries, the event core, battle requests and snapshot data restore. The original encounter/victory/return/menu input route and checkpoints are fixed in the slice manifest, with required extensions and alternatives still unresolved. No broad Phase 1 todo or complete proof domain is closed by this source milestone.
